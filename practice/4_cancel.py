# -*- coding: utf-8 -*-
"""协作式取消（practice/4_cancel.md）

复用 practice/3_adapter.py 的全部零件（文件名以数字开头，无法用普通 import 语句，
用 importlib.import_module('3_adapter') 动态导入，3_adapter.py 一个字不动），
为事件流增加“协作式取消”：
...
def make_run(*, m: int, every: int = 3) -> Run:
    ...
    stream = CancellableEventStream(
        ...
        cancel_check=signal,
        fsm=fsm,
        next_seq=adapter._next_seq,  # 复用 adapter 的 seq 计数器：run.cancelled 也消耗 seq
        every=every,
    )
    return Run(feeder=feeder, adapter=adapter, fsm=fsm, signal=signal, stream=stream)
